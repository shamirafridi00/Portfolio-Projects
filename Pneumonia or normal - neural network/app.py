...
#load model
model =load_model("normal_or_infected.h5")
 
print('@@ Model loaded')
 
 
def pred_human_horse(horse_or_human):
  test_image = load_img(horse_or_human, target_size = (300, 300)) # load image 
  ...
  if pred == 0:
    return "Normal" # if index 0 
  else:
    return "Pneumonia" # if index 1
...
if __name__ == "__main__":
    ...
    app.run(host='127.0.1.6', port=port)
